...
export class Grid {
  ...
  public initBlankGrid(fillWith: string | undefined) {
    this.grid = [];
    for (const i of Array.from(Array(this.numRows).keys())) {
      this.grid.push([]);
      for (const j of Array.from(Array(this.numCols).keys())) {
        this.grid[i][j] = fillWith ?? ' ';
      }
    }
  }

  public setFromSerialized(serialized: string, updateDimensions = true) {
