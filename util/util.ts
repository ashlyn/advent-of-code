...
export function powerSet<T>(inputArr: T[], settings: { proper?: boolean; nonEmpty?: boolean } = {}) {
  let result = inputArr.reduce((subsets, value) => subsets.concat(subsets.map(set => [value, ...set])), [[]] as T[][]);
  if (settings.proper) {
...
export function* getSumSubsets(elems: number[], target: number, level = 0): Generator<number[] | undefined> {
